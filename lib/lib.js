...
lib.join = function (initiatorTel, param) {
	...
	if (param === 'system') {
		lib.sysMsg(initiatorTel, 'join failed, alias can not be system');
		return;
	}
	
	if (param.match(/\d{11}/)) {
		lib.sysMsg(initiatorTel, 'join failed, alias can not look like a phone number');
		return;
	}
	
	var aliasAlreadyExists = lib.db.getList('recipient').filter(function (x) {
