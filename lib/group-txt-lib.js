...
module.exports = function (plivoApi, db, options) {
	...
	self.invite = function (initiatorTel, param) {
		...
		param = param.match(/^1\d{10}$/) && param
			|| param.match(/^\d{10}$/) && ('1' + param)
			|| param.match(/^\d{7}$/) && ('1613' + param)
		;
		
		if (param && param.match(/^1900/)) {
			self.sysMsg(initiatorTel, 'invite failed, 1 (900) xxx-xxxx numbers are not supported');
			return;
		}
		...
		plivoApi.send_message({
			dst: param,
			src: process.env.HOST_NUMBER,
			text: (initiator.alias || initiator.id) + ' has invited you to a group txt.\n\nTo participate respond with ".join Your_Name".'
		});
		
		self.sysMsg(initiatorTel, 'invite sent to ' + param);
	};
